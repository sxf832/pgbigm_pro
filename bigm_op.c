...
cnt_sml_bigm(BIGM *bgm1, BIGM *bgm2)
{
	...
	/* 
	 * Similarity = count of matching bigrams / bigram count of query string (second parameter).
	 * This uses the query string's bigram count as denominator, not max(len1, len2).
	 */
	return ((float4) count) / ((float4) len2);
}
